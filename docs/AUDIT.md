...
| C20 | **No timeouts or retries on any outbound call.** Report generation runs inside the HTTP request, with 400 ms per-keyword sleeps and 3 s sleeps between local-finder pages. | all axios call sites (§3) | Partial: new clients use `src/clients/http.ts` (15 s timeout, 1 retry) since `9fb9c59` and `6771f16` (Phase 3). The legacy call sites are removed in P9. |
